#!/usr/bin/env python3
"""Core single-frame feature extraction for nuPlan scenarios - PURE CYTHON VERSION.

This module is the Cython-optimized implementation with hard dependency on fast_geometry.
NO PYTHON FALLBACK - any import error crashes immediately.

Design goals:
  - Keep identical output to Python reference implementation
  - Use Cython-accelerated geometry operations

HOTSPOT FUNCTIONS IDENTIFIED:
  1. transform_to_ego_frame() - Coordinate transformation (world to ego frame)
  2. _interpolate_points() - Lane interpolation/sampling  
  3. extract_neighbor_agents() - Neighbor agent transformation (uses transform_to_ego_frame)
"""
...
# =================================================================================================
# HARD CYTHON IMPORT - NO TRY/EXCEPT, FAILS IMMEDIATELY IF NOT AVAILABLE
# =================================================================================================
from src.platform import fast_geometry as fg
...
# -------------------------------------------------------------------------------------------------
# Geometry helpers - HOTSPOT 1: Coordinate transformation (world to ego frame)
# -------------------------------------------------------------------------------------------------
...
# =================================================================================================
# HOTSPOT 1: Coordinate transformation (world to ego frame) - CYTHON OPTIMIZED
# =================================================================================================
def transform_to_ego_frame(x, y, ego_x, ego_y, ego_heading):
    """Transform world coordinates to ego frame using Cython-optimized implementation."""
    ...
    if np.isscalar(x):
        ...
        # Use Cython for array inputs
        x_arr = np.asarray(x, dtype=np.float64)
        y_arr = np.asarray(y, dtype=np.float64)
        local_x, local_y = fg.transform_points_to_ego_frame_2d(x_arr, y_arr, float(ego_x), float(ego_y), float(ego_heading))
...
# =================================================================================================
# HOTSPOT 3: Neighbor agent transformation (uses transform_to_ego_frame)
# CYTHON OPTIMIZED - uses batch transformation where possible
# =================================================================================================
...
# =================================================================================================
# HOTSPOT 2: Lane interpolation/sampling - CYTHON OPTIMIZED
# =================================================================================================
def _interpolate_points(line, num_points):
    """Fast lane point interpolation using Cython-optimized implementation."""
    coords = np.asarray(line, dtype=np.float64)
    if len(coords) < 2:
        return np.zeros((num_points, 2), dtype=np.float64)
    return fg.interpolate_lane_points(coords, num_points)
...
def extract_features(
    ...
    *,
    ...
    """Pure feature extraction - CYTHON OPTIMIZED VERSION.

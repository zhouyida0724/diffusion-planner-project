...
def extract_lanes(point, map_api, radius=100, max_lanes=70, ego_heading=0, traffic_light_data=None):
    ...
    for lane_data in lanes_with_dist:
        ...
        # nuPlan map lane_obj.id may be int-like; normalize to string for stable lookup.
...
def extract_route_lanes(
    ...
    # Export-side debug hook: keep last counters in a module global so extract_features can
    # include them in manifest when requested.
